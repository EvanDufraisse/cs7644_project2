...
protected: // ROS Callbacks
    ...
    void sync_callback(const sensor_msgs::PointCloud2ConstPtr pcl_msg, const sensor_msgs::ImageConstPtr img_msg) {
        ...
        pcl_ros::transformPointCloud(base_frame_, pcl_msg->header.stamp, pc_sensor, pcl_msg->header.frame_id, pc_base,
                                     listener_);
        ...
        for (int r = 0; r < img.rows; r += thumb_size_) {
            ...
            for (int c = 0; c < img.cols; c += thumb_size_) {
                ...
                if (type != UNUSABLE) {
                    ...
                    sprintf(filename, "%s/%04ld/%04ld.png", outdir_.c_str(), image_counter_ / 1000,
                            image_counter_ % 1000);
